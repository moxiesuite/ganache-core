...
var tests = function(web3) {
  ...
  before('create and fund personal account', function() {
    return web3.vap.getAccounts()
      .then(function(accs) {
        ...
        return web3.vap.personal.newAccount("password")
      }).then(function(acct) {
  ...
  describe("vap_mining", function() {
    it("should return true", function(done) {
      web3.vap.isMining(function(err, result) {
        if (err) return done(err);
  ...
  describe("vap_sign", function() {
    ...
    before(function( done ){
      ...
      signingWeb3.vap.getAccounts(function(err, accs) {
        if (err) return done(err);
        accounts = accs.map(function(val) {
          return val.toLowerCase();
        });
    ...
    it("should produce a signature whose signer can be recovered", function() {
      ...
      var msgHash = utils.hashPersonalMessage(msg);
  	  ...
  	  return signingWeb3.vap.sign(utils.bufferToHex(msg), accounts[0]).then(sgn => {
    ...
    it("should work if ecsign produces 'r' or 's' components that start with 0", function() {
      ...
      return signingWeb3.vap.sign(msgHex, accounts[0]).then(sgn => {
        sgn = utils.stripHexPrefix(sgn);
        var r = Buffer.from(sgn.slice(0, 64), 'hex');
        var s = Buffer.from(sgn.slice(64, 128), 'hex');
        var v = parseInt(sgn.slice(128, 130), 16) + 27;
  ...
  describe("vap_signTypedData", function() {
    ...
    it("should produce a signature whose signer can be recovered", function(done) {
      var typedData = {"types":{"EIP712Domain":[{"name":"name","type":"string"},{"name":"version","type":"string"},{"name":"chainId","type":"uint256"},{"name":"verifyingContract","type":"address"}],"Person":[{"name":"name","type":"string"},{"name":"wallet","type":"address"}],"Mail":[{"name":"from","type":"Person"},{"name":"to","type":"Person"},{"name":"contents","type":"string"}]},"primaryType":"Mail","domain":{"name":"Vapor Mail","version":"1","chainId":1,"verifyingContract":"0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC"},"message":{"from":{"name":"Cow","wallet":"0xCD2a3d9F938E13CD947Ec05AbC7FE734Df8DD826"},"to":{"name":"Bob","wallet":"0xbBbBBBBbbBBBbbbBbbBbbbbBBbBbbbbBbBbbBBbB"},"contents":"Hello, Bob!"}}
  	  var msg = utils.toBuffer("asparagus");
      var msgHash = utils.hashPersonalMessage(msg);
  ...
  describe('vap_sendRawTransaction', () => {
    ...
    it("should fail with bad nonce (too low)", function(done) {
      ...
      web3.vap.sendSignedTransaction(transaction.serialize(), function(err, result) {
  ...
  describe("contract scenario", function() {
    ...
    it("should add a contract to the network (vap_sendTransaction)", function(done) {
      web3.vap.sendTransaction({
        ...
        web3.vap.getTransactionReceipt(hash, function(err, receipt) {
          if (err) return done(err);
    ...
    it("should represent the block number correctly in the Oracle contract (oracle.blockhash0)", function(done){
      ...
      web3.vap.personal.unlockAccount(accounts[0], "password", function(err, result) {
        var contract = new web3.vap.Contract(JSON.parse(oracleOutput.interface));
        contract.deploy({
          ...
          if (!oracle._requestManager.provider) {
            oracle._requestManager.setProvider(web3.vap._provider);
          }
          web3.vap.getBlock(0, true, function(err, block){
            if (err) return done(err)
    ...
    // describe block.
    it("should read data via a call at a specified blockNumber (vap_call)", function(done){
      var startingBlockNumber = null;
      var call_data = contract.call_data;

      web3.vap.getBlockNumber().then(function(result){

        startingBlockNumber = result;
        return web3.vap.call(call_data)

      }).then(function(result){

        assert.equal(to.number(result), 25, "value retrieved from latest block should be 25");
        return web3.vap.call(call_data, contractCreationBlockNumber)

      }).then(function(result){

        assert.equal(to.number(result), 5, "value retrieved from contract creation block should be 5");
        return web3.vap.getBlockNumber()

      }).then(function(result){

        assert.equal(result, startingBlockNumber, "vap_call w/defaultBlock increased block count");
        return web3.vap.call(call_data);
    ...
    it('should read data via a call when specified blockNumber is "earliest" (vap_call)', function(done) {
      var call_data = contract.call_data;

      web3.vap.call(call_data, "earliest").then(function(result){
        assert.equal(to.number(result), 0, "value retrieved from earliest block should be zero");
        done();
      })
    });

    it('should read data via a call when specified blockNumber is "pending" (vap_call)', function(done){
      var call_data = contract.call_data;

      web3.vap.call(call_data, "pending").then(function(result){
        assert.equal(to.number(result), 25, "value retrieved from pending block should be 25");
        done();
      });
    });

    it("should error when reading data via a call at a non-existent blockNumber (vap_call)", function(done){
      ...
      web3.vap.getBlockNumber().then(function(result){
        ...
        return web3.vap.call(call_data, nonExistentBlock);
  ...
  describe("contract scenario (raw tx)", function() {
    ...
    it("should add a contract to the network (vap_sendRawTransaction)", function(done) {
      web3.vap.sendSignedTransaction(rawTx, function(err, result) {
        if (err) return done(err);
  ...
  describe("vap_getTransactionCount", function() {
    it("should error for non-existent block", function(done) {
      web3.vap.getTransactionCount("0x1234567890123456789012345678901234567890", 9999999, function(err, result) {
  ...
  describe("net_version", function() {
    it("should return a version very close to the current time", function(done) {
      web3.vap.net.getId(function(err, result) {
        if (err) return done(err);
  ...
  describe("personal_newAccount", function() {
    it("should return the new address", function(done) {
      web3.vap.personal.newAccount("password", function(err, result) {
        if (err) return done(err);
  ...
  describe("personal_listAccounts", function() {
    it("should return more than 0 accounts", function(done) {
      web3.vap.personal.getAccounts(function(err, result) {
        if (err) return done(err);
  ...
  describe("personal_unlockAccount", function() {
    it("should unlock account", function(done) {
      web3.vap.personal.unlockAccount(personalAccount, "password", function(err, result) {
        if (err) return done(err);
  ...
  describe("personal_lockAccount", function() {
    it("should lock account", function(done) {
      web3.vap.personal.lockAccount(personalAccount, function(err, result) {
        if (err) return done(err)
        assert.equal(result, true);
        done()
      });
  ...
  /*describe("personal_sendTransaction", function() {
    it("should send transaction", function(done) {
      web3.vap.sendTransaction({value: web3.utils.toWei('5', 'vapor'), from: accounts[0], to: personalAccount }, function(err, result){
        if (err) return done(err)

        web3.vap.personal.sendTransaction({
