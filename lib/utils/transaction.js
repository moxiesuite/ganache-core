const VaporyJsTransaction = require("vaporyjs-tx");
const VaporyJsFakeTransaction = require("vaporyjs-tx/fake");
const vapUtil = require("vaporyjs-util");
const assert = require("assert");
const rlp = require("rlp");
const to = require("./to");

const sign = VaporyJsTransaction.prototype.sign;
const fakeHash = function() {
  ...
  return VaporyJsFakeTransaction.prototype.hash.apply(this, arguments);
};
...
/**
 * vaporyjs-tx's Transactions don't behave quite like we need them to, so
...
function fixProps(tx, data) {
  // vaporyjs-tx doesn't allow for a `0` value in fields, but we want it to
  // in order to differentiate between a value that isn't set and a value
  // that is set to 0 in a fake transaction.
  // Once https://github.com/vaporyjs/vaporyjs-tx/issues/112 is fixed we
  ...
  // Vaporyjs-tx doesn't set the _chainId value whenever the v value is set,
  // which causes transaction signing to fail on transactions that include a
  // chain id in the v value (like vapors.js does).
  ...
  Object.defineProperty(tx, "v", {
    set: (v) => {
      ...
      const sigV = vapUtil.bufferToInt(tx.v);
...
function initData(tx, data) {
  if (data) {
    if (typeof data === "string") {
      data = Buffer.from(vapUtil.stripHexPrefix(data), "hex");
    }
    ...
    if (Array.isArray(data)) {
      ...
      data.forEach((d, i) => {
        self[self._fields[i]] = vapUtil.toBuffer(d);
      });
...
module.exports = class Transaction extends VaporyJsTransaction {
  /**
  ...
  static fromJSON(json, type) {
    ...
    if (json.to) {
      ...
      if (buf.equals(Buffer.from([0]))) {
        ...
        toAccount = vapUtil.setLengthLeft(buf, 20);
      } else {
        toAccount = buf;
      }
    }
    ...
    const options = {
      nonce: vapUtil.toBuffer(to.hex(json.nonce)),
      from: vapUtil.toBuffer(to.hex(json.from)),
      value: vapUtil.toBuffer(to.hex(json.value)),
      gasLimit: vapUtil.toBuffer(to.hex(json.gas || json.gasLimit)),
      gasPrice: vapUtil.toBuffer(to.hex(json.gasPrice)),
      data: data ? Buffer.from(vapUtil.stripHexPrefix(data), "hex") : null,
      to: toAccount,
      v: vapUtil.toBuffer(json.v),
      r: vapUtil.toBuffer(json.r),
      s: vapUtil.toBuffer(json.s)
    };

    const tx = new Transaction(options, type);
    tx._hash = json.hash ? Buffer.from(vapUtil.stripHexPrefix(json.hash), "hex") : null;
    return tx;
  }
   ...
   *
   * This method is nearly identical to vaporyjs-tx hash with the exception of
