...
var wallet = require("vaporyjs-wallet");
...
var sigUtil = require("vap-sig-util");
...
StateManager.prototype.initialize = function(callback) {
  ...
  let defaultBalanceWei = to.hex(Web3.utils.toWei(self.options.default_balance_vapor.toString(), "vapor"));
...
StateManager.prototype.mineOnInterval = function() {
  ...
  // make sure a pending vap_mine doesn't come back and execute mineOnInterval
  // again...
...
StateManager.prototype.queueRawTransaction = function(data, callback) {
  ...
  this._queueTransaction("vap_sendRawTransaction", tx, from, null, callback);
};
...
StateManager.prototype.queueTransaction = function(method, txJsonRpc, blockNumber, callback) {
  ...
  if (method === "vap_sendTransaction" && !this.unlocked_accounts.hasOwnProperty(from)) {
  ...
  if (!isKnownAccount || method === "vap_call") {
...
StateManager.prototype.processNextAction = function(override) {
  ...
  } else if (queued.method === "vap_getStorageAt") {
...
StateManager.prototype.getLogs = function(filter, callback) {
  ...
  // https://github.com/vaporyco/wiki/wiki/JSON-RPC#vap_getlogs
